...
const RISK_COLORS: Record<RiskWindow["risk_level"], { bg: string; text: string; dot: string }> = {
  ...
  high: { bg: "bg-red-900/20", text: "text-red-400", dot: "bg-red-500" },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {snapshot.debts.length > 0 && (
        <section className="rounded-xl bg-[#1e293b] border border-slate-600/50 p-6 shadow-lg shadow-black/20 card-glow transition-all duration-300">
          <h3 className="text-sm font-semibold text-slate-100">Debts</h3>
...
function StatCard({
  ...
  return (
    <div className="rounded-xl bg-[#1e293b] border border-slate-600/50 p-4 shadow-lg shadow-black/20 card-glow transition-all duration-300">
...
function RiskWindowRow({ riskWindow }: { riskWindow: RiskWindow }) {
  ...
  return (
    <div className={`rounded-lg ${colors.bg} border border-slate-600/30 p-3`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          {subscription.leak_reason && (
            <p className="text-xs text-yellow-400/70">{subscription.leak_reason}</p>
          )}
        </div>
      </div>
...
function DebtRow({ debt }: { debt: DebtInfo }) {
  ...
  return (
    <tr>
      ...
      <td className="py-2 text-slate-300">{debt.payoff_months_minimum}</td>
    </tr>
  );
}
